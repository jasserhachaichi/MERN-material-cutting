...
function AllEdges() {
    ...
    const filterStyle = {
        ...
        const fetchData = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`http://localhost:4000/product/edges`, { params: { page: currentPage, limit: PerPage, search: searchInput, status: statusFilter}, withCredentials: true });
...

          {
            ...
            Cell: ({ row }) => (
              <div className="text-end">
                <a href={`/admin/updateedge/${row.original._id}`} className="btn btn-icon btn-warning rounded-0 me-2">
    ...
    return (
        ...
        <div className="card-header border-0 pt-6">
            ...
            <input type="text" data-kt-user-table-filter="search" className="form-control form-control-solid w-250px ps-13" placeholder="Search edge" value={searchInput} onChange={e => setSearchInput(e.target.value)} />
        </div>
