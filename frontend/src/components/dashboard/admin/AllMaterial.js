...
function AllMaterial() {
    ...
    const filterStyle = {
        ...
        const fetchData = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`http://localhost:4000/product/materials`, { params: { page: currentPage, limit: PerPage, search: searchInput, status: statusFilter}, withCredentials: true });
      ...
      const columns = useMemo(
        () => [
                ...
                <div className="symbol rounded-0 symbol-50px overflow-hidden me-3">
                  <a href="#">
                    <div className="symbol-label">
                      <img src={`http://localhost:4000/product/image/material/${row.original.avatarMaterial}` || '/assets/images/Default-profile.jpg'} alt={row.original.material_name} className="w-100" />
                    </div>
                  </a>
                </div>
              </td>
          ...
          },
          {
            ...
            Cell: ({ row }) => (
                getDiscountType(row.original.discount_option)
            )
          },
          {
            Header: 'Discount',
            Cell: ({ row }) => (
                getDiscountValue(row.original.discount_option, row.original.discounted_price, row.original.discounted_percentage)
            )
          },
          {
            Header: 'VAT Amount (%)',
...

          {
            ...
            Cell: ({ row }) => (
              <div className="text-end">
                <a href={`/admin/updatematerial/${row.original._id}`} className="btn btn-icon btn-warning rounded-0 me-2">
    ...
    return (
        ...
        <div className="card-header border-0 pt-6">
            ...
            <div className="card-title">
              ...
              <input type="text" data-kt-user-table-filter="search" className="form-control form-control-solid w-250px ps-13" placeholder="Search material" value={searchInput} onChange={e => setSearchInput(e.target.value)} />
              </div>
