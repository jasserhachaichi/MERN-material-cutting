...
function AllShapes() {
    ...
    const handlePageChange = (newPage) => {
      ...
      const getDiscountValue = (discountOption, fixedDiscount, pourcentageDiscount) => {
        switch (discountOption) {
          case 2:
    ...
    const filterStyle = {
        ...
        const fetchData = async () => {
            setLoading(true);
            try {
                const response = await axios.get(`http://localhost:4000/product/shapes`, { params: { page: currentPage, limit: PerPage, search: searchInput, status: statusFilter}, withCredentials: true });
    ...
    useEffect(() => {
      ...
      const columns = useMemo(
        () => [
                ...
                <div className="symbol rounded-0 symbol-50px overflow-hidden me-3">
                  <a href="#">
                    <div className="symbol-label">
                      <img src={`http://localhost:4000/product/image/shape/${row.original.avatarShapeImg}` || '/assets/images/Default-profile.jpg'} alt={row.original.shapeName} className="w-100" />
                    </div>
                  </a>
                </div>
              </td>
          ...
          },
          {
            ...
            Cell: ({ row }) => (
                getDiscountType(row.original.discountOption)
            )
          },
          {
            Header: 'Discount',
            Cell: ({ row }) => (
                getDiscountValue(row.original.discountOption, row.original.fixedDiscount, row.original.pourcentageDiscount)
            )
          },
          {
            Header: 'VAT Amount (%)',
...

          {
            ...
            Cell: ({ row }) => (
              ...
              <a href={`/admin/updateshape/${row.original._id}`} className="btn btn-icon btn-warning rounded-0 me-2">
      ...
      const handleDelete = async (userId) => {
        ...
        if (result.isConfirmed) {
          try {
            await axios.delete(`http://localhost:4000/product/deleteshape/${userId}`, { withCredentials: true });
    ...
    return (
        ...
        <div className="card-header border-0 pt-6">
            ...
            <input type="text" data-kt-user-table-filter="search" className="form-control form-control-solid w-250px ps-13" placeholder="Search shape" value={searchInput} onChange={e => setSearchInput(e.target.value)} />
        </div>
