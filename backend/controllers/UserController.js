 ...
 module.exports.addstaff = async (req, res) => {
  ...
  try {
    ...
    console.error("Error registering user:", error);
      ...
      const imagePath = req.file ? path.join(__dirname, "..", "uploads", "ProfilesIMG", avatar) : null;
 ...
 * @access  private
 ...
 module.exports.deletestaff = async (req, res) => {
  ...
  try {
    ...
    if (user.image) {
      const imagePathI = path.join(__dirname, "..", "uploads", "ProfilesIMG", user.image);
      console.log(imagePathI);
      if (fs.existsSync(imagePathI)) {
        fs.unlinkSync(imagePathI);
      }
    }
    if (user.imageUrl) {
      const imagePathU = path.join(__dirname, "..", "uploads", "ProfilesIMG", user.imageUrl);
      console.log(imagePathU);
      if (fs.existsSync(imagePathU)) {
        fs.unlinkSync(imagePathU);
      }
    }
  ...
  try {
    ...
    if(user.image != null){
      ...
      if(user.imageUrl != null){
        user.image="http://localhost:4000/api/image/profile/" + user.imageUrl;
      }else{
 ...
 * @access  private
 ...
 module.exports.deleteclient = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ message: "Your account deleted successfully", result: true });
  } catch (error) {
    console.error("Error deleting user:", error);
    return res.status(500).json({ message: "Internal server error.", result: false });
  }
