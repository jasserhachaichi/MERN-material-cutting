...
async function validateRegisterUser(userData) {
  ...
  // Function to get empty attributes of a user
  async function getEmptyAttributes(userId) {
    try {
      ...
      const attributesToCheck = ['firstName', 'lastName', 'email', 'phone', 'addr1', 'addr2', 'town', 'sp', 'postCode', 'country'];
      const emptyAttributes = attributesToCheck.filter(attr => !user[attr] || user[attr].trim() === '');
  
      return { success: true, emptyAttributes };
    } catch (error) {
...
module.exports = {User,validateRegisterUser,validateLoginUser,getEmptyAttributes};
